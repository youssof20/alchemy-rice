...
Alchemy validates, inspects, resolves, exports, and locally imports these documents. Phase 6 also resolves declared dependencies against local package state and reviewed compatibility data. It does not yet apply a complete rice; configuration planning and combined transactional apply remain later phases.
...
alchemy dependency-resolve FILE.rice --sha256 EXPECTED_HASH
alchemy dependency-install FILE.rice DEPENDENCY_ID --plan-token TOKEN --yes
```
...
Dependency resolution is read-only. It reports the declared source and license, local package observation, dated compatibility status, trust tier, exact provider command, privilege requirement, and a plan token. Installation is a separate operation and currently automates only confirmed official distro mappings. Community and manual sources remain visible but require manual review. A rice configuration revert never automatically uninstalls dependencies.
