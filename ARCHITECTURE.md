...
Phase 0 implements only read paths. The capability matrix is immutable for the lifetime of an inspection report, and missing evidence remains `unknown`. Windows is supported for development and unit tests, not as a simulated KDE runtime.

The next phase adds the generic transaction foundation before any broad setting support: targeted snapshots, a durable journal, a single-writer lock, plan/apply/verify/revert, and crash recovery around one color-scheme driver.
